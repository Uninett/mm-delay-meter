...
(
	...
	then
		...
		echo $(date "+%F %H:%M:%S:") "Connecting to WiFi" >> /mnt/sda1/arduino/log.txt
		/mnt/sda1/arduino/wifi_connect.sh
	else
		echo "Wifi connected. Uploading available files."
		echo $(date "+%F %H:%M:%S:") "Wifi connected. Uploading available files" >> /mnt/sda1/arduino/log.txt
	...
	then
		...
		then
			...
			echo $(date "+%F %H:%M:%S:") "Could not connect to WiFi" >> /mnt/sda1/arduino/log.txt
